...
#Name: Carmen I Oliveros Gallegos
...
echo "Hello, I have a friend named ${!someone}"
...
#  '~' expands into the home directory
#  '`' (back tick) executes the command inside it
...
if [ "$name" == "alice" ]; then
    ...
    echo "You're not Alice! You are $name!"
fi
...
cond_test max max
cond_test max notmax
cond_test notmax max


# To check if the strings are equal, use the following command
#
# [[ input1 == input2 ]] 
# In bash [[ ]] is basically a command that evaluates whether or not the
...
aFunction(){
	...
	echo "$firstNum and $secondNum equal $3?"
...
aFunction 17 38 55 #Correct
aFunction 17 38 54 #Incorrect
aFunction 38 17 55 #Correct
aFunction 2 3 6 #Incorrect
...
math_game() {
        #Generates two numbers between 1 and 10
        firstNum=$((RANDOM % 10 + 1))
        secondNum=$((RANDOM % 10 + 1))

        echo "$firstNum and $secondNum equal to $1?"
...
math_game 10
math_game 21 #Will always be false, outside of range
math_game 1 #Ditto^
math_game 5
